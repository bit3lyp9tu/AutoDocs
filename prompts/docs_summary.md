...
@enduml
```
Each time you are writing plantuml code, place the tag `{{TAG_UML_<name>}}` before. Give the tag an unique name, as shown above.
- Aside from the before mentioned uml diagramm types you may choose to represent code in form of the following diagram types: [`{{valid_umls}}`]
- Should you find the opportunity you may also choose to represent some diagrams of the list of non-strictly UML diagrams: [`{{valid_non_umls}}`]
